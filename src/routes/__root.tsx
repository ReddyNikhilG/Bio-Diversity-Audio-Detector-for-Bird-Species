...
function ErrorComponent({ error, reset }: { error: Error; reset: () => void }) {
    ...
    return (
        <div className="flex min-h-screen items-center justify-center bg-background px-4">
            <div className="max-w-md text-center">
                <h1 className="text-xl font-semibold text-foreground font-display">This page didn't load</h1>
                ...
                <div className="mt-6 flex flex-wrap justify-center gap-2">
                    ...
                    >Try again</button>
                    <a href="/" className="btn btn-ghost btn-sm">Go home</a>
                </div>
            </div>
        </div>
    );
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
    head: () => ({
        meta: [
            ...
            { name: "description", content: "Upload audio, separate overlapping bird calls, identify species with a BirdNET + YAMNet + Perch ensemble running on a Colab GPU." },
            { name: "author", content: "BirdSense" },
            { property: "og:title", content: "BirdSense" },
            { property: "og:description", content: "Bioacoustic species detection powered by an AI ensemble." },
...
function SceneSync() {
    ...
    useEffect(() => {
        if (path !== "/") { useScene.getState().setScroll(0); return; }
        const onScroll = () => {
